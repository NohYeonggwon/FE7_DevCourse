...
{
  // 함수 정의
  const concatStringAndNumber = function (str: string, num: number): string {
    return `${str}${num}`;
  };
...
{
  // 함수 정의
  const greet = function (name: string, msg = "Welcome"): string {
    return `${msg} ${name}`;
  };
...
함수 오버로딩을 사용하여 이 기능을 구현하고, 각 타입의 인자를 넘겨 호출해보세요. */
{
  ...
  function getInfo(param: string | number): string | number {
